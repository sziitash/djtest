...
//        if (oSession.fullUrl.Contains("api-theme.meizu.com"))
//        {
//            var fso;
//            var file;
//            fso = new ActiveXObject("Scripting.FileSystemObject");
//            //文件保存路径，可自定义
//            file = fso.OpenTextFile("D:\\fiddlerSessionsRequest\\Sessions.txt",8 ,true, true);
...
//		//过滤无关请求，只关注特定请求
		//&& oSession.fullUrl.Contains("public")
		if (oSession.fullUrl.Contains("api-theme.meizu.com") && !oSession.fullUrl.Contains(":443"))
        {
            oSession.utilDecodeResponse();//消除保存的请求可能存在乱码的情况
            var fso;
            var file;          
            //            var utf8Enc;
            //			utf8Enc = new ActiveXObject("Utf8Lib.Utf8Enc");
            fso = new ActiveXObject("Scripting.FileSystemObject");
            //			file = fso.CreateTextFile("D:\\fiddlerSessionsRequest\\"+oSession.url.Split("?")[0].Replace("/","#").Replace("api-theme.meizu.com","")+".txt", true); //true for overwrite			
            //文件保存路径，可自定义
            //file = fso.OpenTextFile("D:\\fiddlerSessionsRequest\\Responses.txt",8 ,true, false);
            //			file = fso.OpenTextFile("D:\\fiddlerSessionsRequest\\"+oSession.url.Split("?")[0].Replace("/","#").Replace("api-theme.meizu.com","")+".txt",8 ,true);
            file = fso.OpenTextFile("D:\\fiddlerSessionsRequest\\AllApi.txt",8,-2);
            //			file.writeLine("{\"api\":" + "\""+oSession.url.Split("?")[0]+"\","+"\"param\":\""+oSession.url.Split("?")[1]+"\","+"\"type\":"+"\""+oSession.RequestMethod+"\"}");
            //			file.writeLine("Request param^" + oSession.url.Split("?")[1]);
            //			file.writeLine("Request type^" + oSession.RequestMethod);
            try {   
                var type = oSession.fullUrl.Split(":")[0];
                if(oSession.RequestMethod.Equals("GET")){
					file.writeLine("{\"api\":" + "\""+oSession.fullUrl.Split("?")[0]+"\","+"\"param\":\""+oSession.url.Split("?")[1]+"\","+"\"method\":"+"\""+oSession.RequestMethod+"\",\"Response\":" + oSession.GetResponseBodyAsString()+"}");
                }	
                else{
                    file.writeLine("{\"api\":" + "\""+oSession.fullUrl.Split("?")[0]+"\","+"\"param\":\""+oSession.GetRequestBodyAsString()+"\","+"\"method\":"+"\""+oSession.RequestMethod+"\",\"Response\":" + oSession.GetResponseBodyAsString()+"}");	
                }
            }
            catch (err) {             
                if(oSession.RequestMethod.Equals("GET")){
                    file.writeLine("{\"api\":" + "\""+oSession.fullUrl.Split("?")[0]+"\","+"\"param\":\""+oSession.url.Split("?")[1]+"\","+"\"method\":"+"\""+oSession.RequestMethod+"\",\"Response\":" + "\"Error\“}");
                }
                else{
                    file.writeLine("{\"api\":" + "\""+oSession.fullUrl.Split("?")[0]+"\","+"\"Postparam\":\""+oSession.GetRequestBodyAsString()+"\","+"\"method\":"+"\""+oSession.RequestMethod+"\",\"Response\":" + "\"Error\“}");
                }
            }
            file.close();
        }
